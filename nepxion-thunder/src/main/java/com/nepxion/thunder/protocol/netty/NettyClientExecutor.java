...
public class NettyClientExecutor extends AbstractClientExecutor {
    ...
    public void start(String interfaze, ApplicationEntity applicationEntity) throws Exception {
        ...
        connection(interfaze, applicationEntity, barrier);
        
        // 主线程等待连接成功后，第一个await执行，唤醒当前连接线程和主线程
        // 长链接超时，不管第一个await是否执行，过了指定的Timeout时间，直接退出线程
        // barrier.await(properties.getLong(ThunderConstants.NETTY_CONNECT_TIMEOUT_ATTRIBUTE_NAME) * 2, TimeUnit.MILLISECONDS);
        barrier.await();
    }
    
    private void connection(final String interfaze, final ApplicationEntity applicationEntity, final CyclicBarrier barrier) throws Exception {
        ...
        Executors.newCachedThreadPool().submit(new Callable<Object>() {
            ...
            public Object call() throws Exception {
                try {
                            ...
                            .handler(new ChannelInitializer<SocketChannel>() {
                                ...
                                public void initChannel(SocketChannel channel) throws Exception {
                                    channel.pipeline()
                                           ...
                                           .addLast(new NettyClientHandler(executorContainer, properties.getBoolean(ThunderConstants.TRANSPORT_LOG_PRINT_ATTRIBUTE_NAME), properties.getBoolean(ThunderConstants.HEART_BEAT_LOG_PRINT_ATTRIBUTE_NAME)));
                                }
                            });
                    ...
                    LOG.info("Client executor exception", e);
                } finally {
                    ...
                    if (online) {
                        ...
                        connection(interfaze, applicationEntity, null);
                    }
                }
